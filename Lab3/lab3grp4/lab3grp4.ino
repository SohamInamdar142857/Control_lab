...
#define out1            5//Goes to motor
#define out2            6

int m=80;
double Kptheta =-6.5;           //-2.2361;
double Kpalpha =94.9537;         //26.6207;
double Kvtheta =-2.30378;      //-0.5;
double Kvalpha =0.44325;       //3;
//-2.2361   26.6207   -0.9451    3.2913
//-2.8284   44.1628   -1.5769    5.6707
// -4.4721   39.8155   -1.6635    5.0815
//-2.5000   28.2431   -1.0323    3.5117
//-70.7107  639.4295  -29.2903   86.2277
//-38.7298  732.8725  -27.8700   98.8722      
//-31.6228  701.7275  -25.9393   94.6577
//-30.0000  407.9001  -16.5778   54.8975
// -30.0000  592.6844  -22.3271   79.9022
// -0.6047  193.6755   -5.7021   26.9671
//-2.8284   44.1628   -1.5769    5.6707
//-26.4575  350.9485  -14.3264   47.1909
//-30.0000  557.3132  -21.2266   75.1159
//-12.6491   83.9567   -4.1483   11.0593
int currtime=0 , prevtime=0;
double curralpha=0, prevalpha=0, initialalpha;
double currtheta=0, prevtheta=0,initialtheta;
double Vm;
void setup() 
{
...
//create a 16 bit variable to hold the encoders position
  ...
  //set attemps counter at 0 so we can try again if we get bad position    
    ...
    initialtheta = encoderPosition0*2*3.14/16350;
    initialalpha = encoderPosition1*2*3.14/16400;
...
void loop() 
{
  ...
  while(1)
  {
    ...
    curralpha = encoderPosition1*2*3.14/16400 - initialalpha;
    if(curralpha < 0.005 && curralpha > -0.005)
    {   
        initialtheta = encoderPosition0*2*3.14/16350;
        currtheta = 0;
    }
    else
      currtheta = encoderPosition0*2*3.14/16350 - initialtheta;
    
//    currtheta = encoderPosition0*2*3.14/16350 - initialtheta;
    Vm = Kptheta * currtheta + Kpalpha * curralpha + Kvtheta *(currtheta-prevtheta)/(currtime-prevtime) + Kvalpha *(curralpha-prevalpha)/(currtime-prevtime);
 
    if(Vm>=0) 
    { analogWrite(out1,0);
      analogWrite(out2,int(m*Vm));
    }
    else
    {
      analogWrite(out1,-int(m*Vm));
      analogWrite(out2,0);
    }
    Serial.print("Vm=");
    Serial.print(Vm);
    Serial.print("Currthta=");
    Serial.print(currtheta);

    Serial.print("Curralpha=");
    Serial.print(curralpha);
